...
async function getScaleStat(sql, col) {
  ...
  const validCols = ['q1_overall', 'q2_venue', 'q3_ceremony', 'q4_production', 'q5_food'];
  if (!validCols.includes(col)) throw new Error('Invalid column');

  const avgRows = await sql.query(`SELECT AVG(${col}) as avg FROM responses WHERE ${col} IS NOT NULL`);
  const distRows = await sql.query(`SELECT ${col} as val, COUNT(*) as cnt FROM responses WHERE ${col} IS NOT NULL GROUP BY ${col} ORDER BY ${col}`);
  const avg = avgRows.rows[0]?.avg ? Math.round(parseFloat(avgRows.rows[0].avg) * 100) / 100 : 0;
  return { avg, distribution: distRows.rows.map(r => ({ val: r.val, cnt: parseInt(r.cnt) })) };
}
