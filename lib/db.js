...
async function getScaleStatFor(sql, col) {
  let avgRows, distRows;
  if (col === 'q1_overall') {
    avgRows = await sql`SELECT AVG(q1_overall) as avg FROM responses WHERE q1_overall IS NOT NULL`;
    distRows = await sql`SELECT q1_overall as val, COUNT(*) as cnt FROM responses WHERE q1_overall IS NOT NULL GROUP BY q1_overall ORDER BY q1_overall`;
  } else if (col === 'q2_venue') {
    avgRows = await sql`SELECT AVG(q2_venue) as avg FROM responses WHERE q2_venue IS NOT NULL`;
    distRows = await sql`SELECT q2_venue as val, COUNT(*) as cnt FROM responses WHERE q2_venue IS NOT NULL GROUP BY q2_venue ORDER BY q2_venue`;
  } else if (col === 'q3_ceremony') {
    avgRows = await sql`SELECT AVG(q3_ceremony) as avg FROM responses WHERE q3_ceremony IS NOT NULL`;
    distRows = await sql`SELECT q3_ceremony as val, COUNT(*) as cnt FROM responses WHERE q3_ceremony IS NOT NULL GROUP BY q3_ceremony ORDER BY q3_ceremony`;
  } else if (col === 'q4_production') {
    avgRows = await sql`SELECT AVG(q4_production) as avg FROM responses WHERE q4_production IS NOT NULL`;
    distRows = await sql`SELECT q4_production as val, COUNT(*) as cnt FROM responses WHERE q4_production IS NOT NULL GROUP BY q4_production ORDER BY q4_production`;
  } else if (col === 'q5_food') {
    avgRows = await sql`SELECT AVG(q5_food) as avg FROM responses WHERE q5_food IS NOT NULL`;
    distRows = await sql`SELECT q5_food as val, COUNT(*) as cnt FROM responses WHERE q5_food IS NOT NULL GROUP BY q5_food ORDER BY q5_food`;
  } else {
...
export async function getStats() {
  ...
  for (const q of ['q1_overall', 'q2_venue', 'q3_ceremony', 'q4_production', 'q5_food']) {
    scaleStats[q] = await getScaleStatFor(sql, q);
  }
